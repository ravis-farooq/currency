      ...
      >
        <thead>
          <tr>
            ...
            {data[0]?.currency?.map((item: Record<string, string>) => (
              <th className="table-style">
                ...
                >
                  ...
                  <svg
                    cursor={"pointer"}
                    role="button"
                    onClick={() => handleRemoveCurrency(item.name)}
                    width="17"
                    height="18"
                    viewBox="0 0 17 18"
                    fill="none"
            ...
            data?.map((item, index) => {
              return (
                <tr key={index.toString()} className="table-style">
                  <td className="table-style">{item.date}</td>
                  {item?.currency?.map(
                    (elem: Record<string, string>, i: number) => {
                      return (
                        <>
